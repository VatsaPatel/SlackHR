...
export default class Calendar extends PureComponent {
	...
	getDates = (): Array<Moment> => {
		const showDaysBeforeCurrent = 10;
		// And after
		const showDaysAfterCurrent = 10;
	...
	scrollToCurrentDay = () => {
		...
		// Minimal possible X position value to prevent scrolling before the first
		// Maximum possible X position value to prevent scrolling after the last day
		// no scrolling if there's nowhere to scroll
		...
		if (scrollToX < 1000) {
			scrollToX = 0;
		} else if (scrollToX > 1000) {
			// Do not scroll over the right edge
			scrollToX = 1000;
		}

		this._scrolldiv.scrollTo({ x: scrollToX });
	};
	...
	render() {
		...
		return (
			<div className={styles.Container}>
				<span className={styles.visibleMonthAndYear}>{visibleMonthAndYear}</span>
				<Dates dates={dates} currentDateIndex={currentDateIndex} onSelectDay={this.onSelectDay} onRenderDay={this.onRenderDay} />
			</div>
		);
	}
}
